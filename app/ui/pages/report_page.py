...
class ReportPage(QWidget):
    """
    Genel envanter, günlük hareket ve satış istatistikleri raporlarını sekmeler halinde gösteren sayfa.
    """
    CALENDAR_STYLE = """
    ...
    def _create_statistics_tab(self):
        ...
        self.start_date_edit = QDateEdit(QDate.currentDate().addDays(-30))  # Son 30 gün
        ...
        # 2. Sonuçların Gösterileceği Alan
        ...
        self._calculate_and_show_statistics()  # İlk açılışta son 30 günü hesapla

    def _calculate_and_show_statistics(self):
        """Seçilen tarih aralığı için istatistikleri hesaplar ve etiketlerde gösterir."""
    ...
    def _create_daily_activity_tab(self):
        """Günlük işlem detaylarını görmek için bir sekme oluşturur."""
        tab = QWidget()
        layout = QVBoxLayout(tab)
        layout.setContentsMargins(15, 15, 15, 15)
    ...
    def _create_inventory_summary_tab(self):
        """Genel envanter özeti için bir sekme oluşturur."""
        tab = QWidget()
        layout = QVBoxLayout(tab)
        layout.setContentsMargins(15, 15, 15, 15)
    ...
    def _load_inventory_data(self):
        """Genel envanter verilerini yükler ve arayüzü günceller."""
        ...
        for cins, toplam_stok in type_counts:
            self.type_counts_model.appendRow([QStandardItem(cins), QStandardItem(str(toplam_stok))])

    def _open_daily_detail_dialog(self, date: QDate):
        """Seçilen tarihin detaylarını gösteren yeni diyalog penceresini açar."""
        detail_dialog = DailyDetailDialog(selected_date=date, parent=self)
        detail_dialog.exec()

    def showEvent(self, event):
        """Bu sayfa her gösterildiğinde verileri yeniler."""
